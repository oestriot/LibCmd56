...
*   this is not incuded here because its result is always the same, calculating it every time would be slower, 
...
void derive_cmac_packet18_packet20(AES_ctx* ctx, uint8_t* data, uint16_t response_size, uint8_t* output, size_t size) {
	uint8_t cmac_input[0x50] = { 0 };

	// aes-128-cmac the whole thing
	memcpy(cmac_input, &response_size, sizeof(response_size));
